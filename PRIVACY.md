...
- Avatars are stored as compressed image data in the user's profile metadata (`meta/profile`).
...
Account in Settings. Account deletion erases all application subcollections
(tracker configurations, history logs, daily dated records, profile metadata),
the profile document, and the Firebase Authentication account. If final
